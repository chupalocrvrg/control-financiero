...
// 8. Update UI fields
// Safely remove the "Nombre del Artículo" input block:
// We look for:
/*
                <div>
...
*/
content = content.replace(
  /<div>\s*<label className="text-\[10px\] font-black text-neutral-400 uppercase tracking-widest block mb-1\.5">Nombre del Artículo \*<\/label>\s*<input\s*type="text"\s*required\s*placeholder="Ej. Taladro Percutor 1\/2..."\s*value=\{formData\.name\}\s*onChange=\{\(e\) => setFormData\(\{ \.\.\.formData, name: e\.target\.value \}\)\}\s*className="[^"]*"\s*\/>\s*<\/div>/,
...
// Safely replace the "Número de Serie" input block:
/*
                  <div>
...
*/
content = content.replace(
  /<div>\s*<label className="text-\[10px\] font-black text-neutral-400 uppercase tracking-widest block mb-1\.5">Número de Serie \(Opcional\)<\/label>\s*<input\s*type="text"\s*placeholder="Ej. SN-3948572"\s*value=\{formData\.series\}\s*onChange=\{\(e\) => setFormData\(\{ \.\.\.formData, series: e\.target\.value \}\)\}\s*className="[^"]*"\s*\/>\s*<\/div>/,
  `<div>
...
// We should also remove "quantity" disabled state from editing article if we want them to enter series? No, we don't allow modifying quantity directly from "Editar", but if requiresSeries is checked they need to provide the total series.
// We handled this in targetQuantity calculation.
